...
\begin{itemize}
\item \textbf{kernel}: $\text{ker}(L) = \{ \vx \in \R^n | L(\vx) = \vec{0}\}$
\item \textbf{range}: $R(L) = \{ L(\vx) \in \R^m | \vx \in \R^n\}$
\end{itemize}
\todo{ Range, kernel and other stuff (colspace, etc)}
...
\end{note}
\todo{Operations on Linear Mapping}
